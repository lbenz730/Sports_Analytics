v <- read.csv("Results/NCAA_MLAX_Results_2018.csv", as.is = T)
...
x <- x[-these, ]
x <- x[x$opponent != "Cornell College" & x$opponent != "St. Thomas Aquinas" & x$opponent!= "Walsh" & x$opponent != "Alderson Broaddus" & x$opponent != "Chowan",]
x$team <- gsub("&#x27;", "'", x$team)
...
suppressWarnings(ivy.sim(5000, games))
